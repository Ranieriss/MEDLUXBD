...
export async function renderObras(view) {
  ...
  const showModal = (item = null) => {
    ...
    content.querySelector('#save').onclick = async () => {
      ...
      const validationError = validateObra(payload);
      if (validationError) {
        await tryAuditLog({ action: 'ERROR', entity: 'obras.validation', severity: 'WARN', details: { message: validationError } });
        return toast(validationError, 'error');
      }
      try {
        const saved = item
          ? await updateObra(item.id, payload)
