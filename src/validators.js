...
export function validateEquipamento(payload) {
  if (!required(payload.codigo)) return 'Código é obrigatório.';
...
export function validateVinculo(payload) {
  if (!required(payload.equipamento_id) || !required(payload.user_id) || !required(payload.inicio_em)) return 'equipamento_id, user_id e inicio_em são obrigatórios.';
...
export function validateMedicao(payload) {
  if (!required(payload.equipamento_id) || !required(payload.user_id) || !required(payload.tipo) || !required(payload.unidade) || !required(payload.medido_em) || !required(payload.data)) {
    return 'Campos obrigatórios: equipamento_id, user_id, tipo, unidade, medido_em e data.';
  }
